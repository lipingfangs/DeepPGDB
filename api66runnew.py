...
scriptindex=sourcedic["scriptindex"]
# 模型 API 的地址
...
def scriptindexadd(line):
    insert_pos = line.find("seqacq")
    if insert_pos != -1:
        line = line[:insert_pos] + scriptindex +"/"+ line[insert_pos:]
    print(line)
    return line
...
#    if "tempgene" in model_output:
        ...
        if ifonlyonegene.find("only one")!= -1:
           # 执行命令    
            ...
            commands = scriptindexadd(commands)
...
def population_hapcommand(model_output):
    if "#haplotype" in model_output:
        commands = f"python {scriptindex}/halotypepie.py output_genotypes.previous.csv poplutaion.info"
...
def Protein_elecommand(model_output):
    if "#Protein" in model_output:
        commands = f"python {scriptindex}/calculatepep.py --fasta tempgeneprevious.fa  --csv tempgeneprevious.fa.out"
        print("Executing command:", commands)
        os.system(commands.strip())
...
def seqcomparsion(model_output):
    if "forsequencealn.fasta" in model_output:
       commands = model_output
       os.system(commands.strip())
       os.system(f"python {scriptindex}/seqcomp.py")
       file_content = read_file_with_cat("tempgtfgene.gtf")
      ...
      #  print("Executing command:", commands)
       ...
       # os.system(commands.strip())
        if len(keyword.split()) > 1 or  len(keyword) > 16:
            commands = f'python {scriptindex}/searchfunction.py {getfile} "{keyword}" -t 75 > outgene.ann'
            os.system(commands.strip())
            print(commands)
...
def querygenomeinfo(model_output,queryhistory,user_input):
    if "querygenomeinfo.list" in model_output:
        ...
        InfoModel = sourcedic["InfoModel"]
        Infofile = sourcedic["Infofile"]
        commands = model_output
        commands =    f"echo '{user_input}' > querygenomeinfo.list ;python {scriptindex}/querygenomeinfo.py {MODEL_API_URL} {InfoModel} {Infofile} > querygenomeinfo.list.result"
        print(commands)
        os.system(commands)
...
def variation_command(model_output):
    if "population" in model_output:
        ...
        # 执行命令
        os.system(commands.strip())
        os.system(f"python {scriptindex}/buildvarmatrix.py  output_variation.ped output_variation.map")
